...
function PrivacyManagement({ customer, onBack }) {
  ...
  const smsConsent = consentRecord?.find(c => c.consent_type === 'sms_marketing')
  ...
  const updateConsent = async (consentType, consentGiven) => {
    try {
      ...
      setNotification({ 
        type: 'success', 
        message: `Consenso ${consentType} ${consentGiven ? 'attivato' : 'disattivato'} con successo` 
      })
      
      // Ricarica i consensi
      await loadConsentRecord()
    } catch (error) {
      console.error('Errore:', error)
      setNotification({ type: 'error', message: 'Errore nell\'aggiornamento del consenso' })
    }
