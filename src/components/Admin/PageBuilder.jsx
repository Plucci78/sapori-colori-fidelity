...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error("Uncaught error in PageBuilder:", error, errorInfo);
    ...
    if (this.state.hasError) {
      return (
        <div style={{ padding: '20px', border: '1px solid red', color: 'red' }}>
          <h1>Si è verificato un errore nel Page Builder.</h1>
          <p>Per favore, controlla la console del browser per maggiori dettagli.</p>
...
export default function PageBuilder() {
  console.log('PageBuilder component rendering...');
  const editorOptions = {
  ...
  console.log('StudioEditor options:', editorOptions);
  return (
    <ErrorBoundary>
      <StudioEditor
